  ...
  void read_data(const char* filename)
  {
      ...
      for (int i = 0; i < nrows; ++i)
      {
          ...
          for (int j = 0; j < nrows; ++j)
          {
              ...
              {
                 this->dist_xy[i][j] = sqrt( (xytemp[i][0] - xytemp[j][0]) * (xytemp[i][0] - xytemp[j][0]) + (xytemp[i][1] - xytemp[j][1]) * (xytemp[i][1] - xytemp[j][1]) );
              }
          }
      }
...
void SOCMAX(const CData& dat)
{
  ...
  NumVar2DMatrix Q(env, N);
  Num2DMatrix Q_val(env, N);
  ...
  for (int i = 0; i < N; i++)
  {
      ...
      Q[i] = IloNumVarArray(env, N, 0, IloInfinity, ILOFLOAT);
      Q_val[i] = IloNumArray(env, N);

      V[i] = IloNumVarArray(env, N, 0, IloInfinity, ILOFLOAT);
  ...
  for (int i = 0; i<N; ++i)
  {
      ...
      for (int j = 0; j<N; ++j)
      {
          ...
          sprintf(varname,"Q(%d,%d)",i,j);
          Q[i][j].setName(varname);
  ...
  // R_ij - sum(k,l) u_ijkl x_kl - uc_ij = 0
  for (int i = 0; i<N; ++i)
  {
      for (int j = 0; j<N; ++j)
      {
          if (i != j)
          {
             ...
             for (int k = 0; k<N; ++k)
             {
                 for (int l=0; l<N; ++l)
                 {
                     rR -= dat.u[i][j][k][l] * X[k][l];
                 }
             }
             model.add(rR == dat.uc[i][j]);
             rR.end();
          }
      }
   }
   ...
   for (int i = 0; i<N; ++i)
   {
       for (int j = 0; j<N; ++j)
       {
           ...
           {
              IloExpr conic1(env);
              conic1 += (V[i][j] + R[i][j]) * (V[i][j] + R[i][j]);
	            conic1 -= V[i][j] * V[i][j];
      	      conic1 -= R[i][j] * R[i][j];
	            for (int k = 0; k < N; ++k)
	            {
            		  for (int l = 0; l < N; ++l)
		              {
             		      conic1 -= 2.0 * dat.u[i][j][k][l] * X[k][l] * X[k][l];
		              }
	            }
              model.add(conic1 >= 0.0);
              conic1.end();
           }
      }
   }
   ...
   cout << endl;
   cout << endl;
   cout << "Second order conic formulation: maximization (theirs)" << endl;
   cout << endl;
   cout << "Captured marketshare " << fixed << setprecision(2) << marketshare_lost << " %" << endl;
   cout << "Lost marketshare     " << 100.0 - marketshare_lost << " %" << endl;
   cout << "Captured market      " << setprecision(2) << UB << endl;
   cout << "Lost market          " << setprecision(2) << dat.total_flow - UB << endl;
...
{
   ofstream outfile;
   outfile.open("mpmax.dat",std::ofstream::app);
   outfile << execname;
   outfile << ","; 
   outfile << filename;
   outfile << ","; 
