...
void SOCP(const CData& dat)
{
  ...
  NumVar2DMatrix Gamma(env, N);
  Num2DMatrix Gamma_val(env, N);
  ...
  for (int i = 0; i < N; i++)
  {
      ...
      Z[i] = IloNumVarArray(env, N, 0, IloInfinity, ILOFLOAT);
      Z_val[i] = IloNumArray(env, N);

      Gamma[i] = IloNumVarArray(env, N, 0, 1, ILOFLOAT);
      Gamma_val[i] = IloNumArray(env, N);
      for (int j = 0; j < N; j++)
      {
  ...
  for (int i = 0; i<N; ++i)
  {
      for (int j = 0; j<N; ++j)
      {
          sprintf(varname,"g(%d,%d)",i,j);
          Gamma[i][j].setName(varname);
          objective += dat.h[i][j] * Gamma[i][j];  
      }
  }
  ...
  */
  ...
  for (int i = 0; i<N; ++i)
  {
      for (int j = 0; j<N; ++j)
      {
          if (i != j)
          {
             ...
             for (int k = 0; k<N; ++k)
             {
                 for (int l=0; l<N; ++l)
                 {
                     sumz += dat.pi[i][j][k][l] * X[i][j][k][l];
                 }
             }
             model.add(sumz + 1 == Z[i][j]);
             sumz.end();
          }
      }
   }
   // conic constraint gamma z >= 1
   for (int i = 0; i<N; ++i)
   {
       for (int j = 0; j<N; ++j)
       {
   ...
   cout << endl;
   cout << endl;
   cout << "Second order conic formulation (ours) " << endl;
   cout << endl;
   cout << fixed << setprecision(2);
   cout << "Captured marketshare " << setprecision(2) << 100.0 - marketshare_lost << " %" << endl;
...
{
   ofstream outfile;
   outfile.open("spmin.dat",std::ofstream::app);
   outfile << execname;
   outfile << ","; 
   outfile << filename;
   outfile << ","; 
